...
export interface ExtractorPromptInput {
  ...
  examMeta?: ExamMeta;
  /** runtime-injected reference doc for synthetic_division / pascal tables */
  referenceDoc?: string;
}

const EXTRACTOR_SYSTEM = `너는 NGD V3 시험지 문제 추출 전문 에이전트다. 문제 이미지 1장을 받아서 구조화된 JSON으로 추출한다.
...
- subtopic 필드에는 반드시 unit_classification.json의 topics 값을 그대로 사용
...
다음 형식의 JSON만 반환한다 (마크다운 없이, JSON만):
{
  ...
  "score": "<배점 문자열>",
  "difficulty": "하" | "중" | "상" | "킬",
  "subtopic": "<unit_classification.json의 topics 값>",
  "has_figure": <boolean>,
  "figure_info": {
...
## 표 type 추출 — synthetic_division / pascal reference 문서

표 type 이 \`synthetic_division\` 또는 \`pascal\` 인 경우, 각 셀 값의 형식은 첨부된 reference 문서 명세를 정확히 따른다. Reference 문서가 prompt 에 첨부되어 있으면 그 명세대로 출력하라.
...
export function buildExtractorPrompt(input: ExtractorPromptInput): { system: string; user: string } {
  // Inject reference doc into system prompt if provided
  let system = EXTRACTOR_SYSTEM;
  if (input.referenceDoc && input.referenceDoc.trim()) {
    system = system.replace(
      "[REF_DOC_SECTION]",
      `\n\n---\n### 첨부 Reference 문서 (syn_div / Pascal 셀 형식)\n\n${input.referenceDoc.trim()}\n---`,
    );
  } else {
    system = system.replace("[REF_DOC_SECTION]", "(reference 문서 미첨부 — 기본 판단으로 추출)");
  }
