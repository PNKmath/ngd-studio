...
프롬프트에 **"이미지 전용 모드"**가 명시된 경우 → 아래 `### 이미지 전용 모드` 절차를 따른다.
그 외에는 `### 1. PDF → JPG 변환`부터 시작한다.

### 이미지 전용 모드 (PDF 없음)

원본 PDF 없이 문제별 개별 이미지만 제공된 경우의 절차:

1. 프롬프트에 나열된 문제 이미지를 **번호 순서대로** Read 도구로 읽는다
2. 각 이미지에서 문제 1개를 추출한다 (번호, 유형, 본문, 선지, 배점, 정답, 해설 등)
3. 시험 정보(학교명, 학년, 과목 등)는 이미지에서 파악할 수 있으면 추출하고, 없으면 `[UNCLEAR]`로 남긴다
4. 이미지가 제공된 문제만 JSON에 포함한다 (없는 번호는 건너뜀)
5. 이후 수식 변환, JSON 저장, 검증은 동일하게 수행

**핵심**: 이미지 1개 = 문제 1개이므로 다른 문제와 혼동이 없다. 읽을 수 없는 부분은 `[UNCLEAR]`로 표시하고 절대 창작하지 않는다.
...
### 3. 문제별 개별 이미지로 2차 검증 (v2)

프롬프트에 **문제별 개별 이미지** 경로가 제공된 경우, PDF에서 1차 추출한 결과를 개별 이미지로 **반드시 2차 검증**한다.

**절차**:
1. PDF에서 모든 문제를 1차 추출한다 (기존 방식)
2. 개별 이미지가 제공된 문제에 대해 Read 도구로 해당 이미지를 읽는다
3. **1차 추출 결과와 개별 이미지를 비교**하여 불일치를 찾는다
4. 불일치가 발견되면 **개별 이미지의 내용으로 수정**한다 (개별 이미지는 해당 문제만 크롭한 것이므로 더 정확함)

**창작 방지 규칙 (최우선)**:
- **읽을 수 없는 부분은 `[UNCLEAR]`로 표시**하고, 절대 추측하거나 창작하지 않는다
- PDF에서 읽은 내용이 불확실할 때, 비슷한 유형의 다른 문제를 참고하여 내용을 만들어내는 행위를 **절대 금지**한다
- 개별 이미지에도 없고 PDF에서도 읽을 수 없으면 → `[UNCLEAR]`로 남긴다
- 문제의 주제가 비슷하더라도 (예: 둘 다 분산을 구하는 문제) **각 문제는 완전히 독립적**이며, 한 문제의 내용을 다른 문제에 차용하지 않는다
- 선지(①~⑤)의 값은 반드시 원본에서 읽어야 하며, 계산 결과로 유추하여 선지를 만들지 않는다

**검증 체크리스트**:
- [ ] 개별 이미지가 제공된 모든 문제 번호가 추출 결과에 존재하는가?
- [ ] 각 문제의 본문이 개별 이미지와 일치하는가?
- [ ] 선지의 값이 개별 이미지와 일치하는가?
- [ ] 배점이 일치하는가?
- [ ] 수식이 정확히 동일한가?

### 4. 수식 변환 (hwp-equation 규칙)
...
### 5. JSON 저장 — Interleaved Parts 포맷
