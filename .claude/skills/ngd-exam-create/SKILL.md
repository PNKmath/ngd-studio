...
**재시도 루프** (문제별, 최대 3회): `ngd-studio/server/stages/stagePlan.ts` (`applyVerifierRetry`) 참조.
...
취합 로직은 `ngd-studio/server/stages/examData.ts` (`aggregateVerifiedProblems`) 참조.
...
### Figure 처리
`figureRunner`(`ngd-studio/server/stages/figureRunner.ts`)가 `figure_processor.py`를 호출한다.
crop boundary 불확실 케이스(`boundary_uncertain=true`)만 `ngd-exam-figure` agent에 위임한다.
