...
| `V3 resume --from=review_extract` | 추출 검증 resume | 추출 유지 + 검증 화면부터 (solver 미실행 시) |
...
| `V3 resume --from=review` | 검증 resume | 최종 사용자 검증 단계부터 |

**프론트엔드 버튼 매핑** (SSE 호출 시 프롬프트로 전달):

| 버튼 | 프롬프트 |
|------|---------|
| [문제N 이미지 재정리] | `V3 resume --q=N --from=cleaned` |
| [문제N 재추출] | `V3 resume --q=N --from=extractor` |
| [추출 결과 검증] | `V3 resume --from=review_extract` |
...
def cleanup_from_stage(question_nums, from_stage):
    ...
    # 단계 순서: cleaned → extractor → solver → verifier
    stage_files = {
...
4문제씩 배치로 nano-banana를 **동시에** 호출한다:
...
문제를 **4개씩 배치**로 묶어 extractor를 실행한다. **Solver는 아직 시작하지 않는다.**
...
배치 1(Q1~Q4) → 배치 2(Q5~Q8) → ... → 마지막 배치까지 **extractor만** 실행.
...
전체 문제를 **4개씩 배치**로 묶어 처리. 각 배치 내에서는 **동시에** 호출한다.
...
#### 5-2. 최종 검증 화면 출력 + Figure 병렬 시작

JSON 취합 후 **두 가지를 동시에** 진행한다:

**A) 최종 검증 화면 출력**

각 문제에 대해 **원본 이미지 + 정리본 이미지 + 해설 요약**을 보여준다. Read 도구로 두 이미지를 모두 읽어 표시하고, verified JSON에서 핵심 정보를 텍스트로 요약한다.

```
=== 문제 {N}번 최종 검증 ===

[원본] Read 도구로 inputs/시험지 제작/question_images/q{N}.png 표시
[정리본] Read 도구로 inputs/시험지 제작/question_images/cleaned/q{N}.png 표시
...
이 단계에서는 주로 **해설 품질**을 검토한다. 문제 본문/정답 수정은 Step 3.5에서 완료되었으므로, 필요 시 `q{N}_verified.json`을 수정하고 `exam_data.json`을 재취합한다.

모든 문제를 순서대로 출력한 후, 사용자에게 확인을 요청한다:

```
모든 문제 최종 검증 화면을 출력했습니다.
...
다음 **두 조건이 모두 충족**되어야 Step 6(Builder)로 진행한다:
