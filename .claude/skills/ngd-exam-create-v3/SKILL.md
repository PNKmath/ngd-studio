...
| `V3 resume --q=3,7 --from=extractor` | 지정 resume (복수) | 3,7번 extractor 병렬 재실행 |
...
| 버튼 | 프롬프트 |
|------|---------|
| [문제N 이미지 교체] | `V3 resume --q=N --from=image_replace` |
...
| [문제N 그림 재처리] | `V3 resume --q=N --from=figure` |
| [확인] | `V3 resume --from=confirm` |
...
| [문제N 재추출] 클릭 | `V3 resume --q=N --from=extractor` → 해당 문제 재추출 후 Step 3.5로 복귀 (복수 선택 시 `--q=N,M`) |
| [문제N 이미지 교체] 클릭 | 프론트엔드가 새 이미지를 `q{N:02d}.png`로 저장 → `V3 resume --q=N --from=image_replace` |
...
**job 종료**: figure 백그라운드 시작 후 Claude는 job을 종료한다(status="done"). 프론트엔드가 `figure_status.json`을 polling하여 완료 시 `outputs/images/prob{N}_final.png` 이미지를 표시한다. 사용자가 이미지를 확인하고 [확인] 버튼을 누르면 `V3 resume --from=confirm`이 전송된다.

#### 5-3. --from=confirm 처리

`V3 resume --from=confirm` 수신 시:
