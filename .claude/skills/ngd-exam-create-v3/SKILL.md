...
### Step 4: Phase 1 완료 — JSON 취합 + 사용자 검증 + Figure 병렬 처리
...
#### 4-2. 사용자 검증 화면 출력 + Figure 병렬 시작

JSON 취합 후 **두 가지를 동시에** 진행한다:

**A) 사용자 검증 화면 출력**

각 문제에 대해 **원본 이미지 + 정리본 이미지 + 추출/해설 요약**을 보여준다. Read 도구로 두 이미지를 모두 읽어 표시하고, verified JSON에서 핵심 정보를 텍스트로 요약한다.
...
**원본과 정리본을 둘 다 보여주는 이유**: nano-banana가 이미지를 재생성하는 과정에서 숫자/수식이 변조될 수 있으므로, 사용자가 원본과 비교하여 정리본의 정확성을 검증할 수 있도록 한다.
