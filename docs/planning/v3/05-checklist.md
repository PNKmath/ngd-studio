...
- [ ] `.claude/agents/ngd-exam-extractor.md` 작성
  - [ ] 역할/입출력 정의
  - [ ] 이미지 기반 추출 규칙 (parts 배열 형식)
  - [ ] HWP 수식 문법 참조 (hwp-equation 스킬)
  - [ ] 단원분류표 참조 규칙
  - [ ] JSON 출력 형식 (number, type, score, parts, choices, answer, figure_info)
  - [ ] [UNCLEAR] 처리 규칙
- [ ] 단독 테스트: 이미지 1장 → JSON 추출 확인

### 1-2. ngd-exam-verifier (신규)
- [ ] `.claude/agents/ngd-exam-verifier.md` 작성
  - [ ] 검증 항목 A: 수학적 정확성 (답 역산, 중간 계산, 완결성)
  - [ ] 검증 항목 B: 교과 범위 준수
  - [ ] 검증 항목 C: 서식 규칙 (guidelines-answer.md)
  - [ ] 검증 항목 D: 원본 이미지 대조
  - [ ] pass/fail 출력 형식
  - [ ] fail 시 feedback 형식 (solver 재호출용)
- [ ] 단독 테스트: 정상 해설 → pass, 오류 해설 → fail + feedback

### 1-3. ngd-exam-solver 수정
- [ ] 문제별 독립 호출 지원 (입력 형식 변경)
- [ ] 교과 순서 컨텍스트 입력 처리
- [ ] 난이도별 줄 수 제한 제거
- [ ] verifier feedback 반영하여 재생성 지원
...
- [ ] `.claude/skills/ngd-exam-create-v3/SKILL.md` 작성
  - [ ] Phase 1 병렬 처리 흐름 (4개씩 배치)
  - [ ] extractor → solver → verifier 루프 (최대 3회)
  - [ ] 교과 컨텍스트 생성 로직
  - [ ] Phase 2 순차 처리 (figure → builder → checker)
  - [ ] JSON 취합 로직
  - [ ] 에러 처리 (문제별/배치별/전체)
  - [ ] 결과 리포트 출력 형식
...
### 3-1. create-v3 페이지
- [ ] `ngd-studio/app/create-v3/page.tsx` 작성
  - [ ] 시험지 메타 정보 입력 (학교, 학년, 과목, 범위)
  - [ ] QuestionSlotGrid 연동
  - [ ] 제작 시작/중단 버튼
  - [ ] 결과 카드 + 다운로드
  - [ ] 참고사항 GuidePanel
...
### 3-3. PipelineView 변경
- [ ] V3 스테이지 추가 (extractor, solver, verifier, figure, builder, checker)
...
### 4-1. prompts.ts
- [ ] `buildCreateV3Prompt()` 함수 추가
- [ ] 문제 이미지 경로 목록 + 메타 정보 포맷팅
- [ ] WSL 경로 변환 적용

### 4-2. sse.ts
- [ ] `mode === "create-v3"` 라우팅 추가
- [ ] V3 SSE 이벤트 (`question`, `question_result`) 변환
- [ ] 스테이지 감지 패턴 추가 (extractor, verifier)

### 4-3. claude.ts
- [ ] `agentTypeToStage`에 extractor, verifier 추가
- [ ] 텍스트 패턴 매핑 추가

### 4-4. useJobRunner.ts
- [ ] `startJob("create-v3", ...)` 지원
- [ ] V3 SSE 이벤트 핸들링
...
| `.claude/agents/ngd-exam-solver.md` | solver 기존 규칙 (수정 대상) |
