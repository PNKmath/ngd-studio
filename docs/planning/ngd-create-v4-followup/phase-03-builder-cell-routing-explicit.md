...
- [x] `_inject_cell_value` 시그니처에 `force_text` 추가 + mutually exclusive 가드 + 기존 동작 호환 (둘 다 False = heuristic)
- [x] tables.py / shapes.py 의 모든 셀 주입 호출지점 audit — 라우팅 정책 명시 (force_equation / force_text / heuristic) 주석 또는 인자로 표기
- [x] tables.py 상단에 generator 라우팅 컨벤션 docstring 추가
- [x] `python3 build_hwpx.py "inputs/시험지 제작/.v3cache/exam_data.json" outputs` exit 0 (회귀 없음)
- [x] `python3 tools/build_template_showcase.py` exit 0 + 시각 회귀 없음 (특히 syn_div equation 유지 확인)
...
### 1회차 (2026-05-19 20:35 KST) — 완료
**상태**: completed
**소요 시간**: 약 10분
**진행 모델**: claude-sonnet-4-6

#### 요약
`_inject_cell_value`에 `force_text` 파라미터 + mutually exclusive 가드 추가. `make_pascal_table`이 이항계수를 text로 잘못 라우팅하던 버그를 `force_equation=True`로 수정. `make_increase_decrease_table`, `make_choice_table`, `make_data_table_xml` (normal_dist/probability) 등 heuristic 유지 호출지점에 정책 주석 표기. `tables.py` 상단에 generator 라우팅 컨벤션 docstring 추가.

#### 변경 파일
- `tables.py` (수정, +35/-6줄)
- `shapes.py` — audit 결과 `_inject_cell_value` 직접 호출 없음, 변경 없음

#### 검증 결과
- [x] build_hwpx.py 회귀: `python3 build_hwpx.py "inputs/시험지 제작/.v3cache/exam_data.json" outputs` → exit 0
- [x] build_template_showcase.py: `python3 tools/build_template_showcase.py` → exit 0
- [x] syn_div equation 라우팅 유지: equation=9, text=0 → OK
- [x] force_text 신규 동작: `_inject_cell_value(cell, '1', force_text=True)` → `<hp:t>` 생성, `<hp:equation` 없음 → OK
- [x] mutual exclusion 가드: `force_equation=True, force_text=True` → ValueError 정상 발생

#### 추가 발견사항
`make_pascal_table` 이항계수 셀이 기존에는 heuristic(text)으로 라우팅되었음 — 정수("1","3","6" 등)는 변수 없이 순수 숫자라 text로 떨어짐. `force_equation=True` 적용으로 HYhwpEQ 폰트 렌더링 보장. shapes.py는 `_inject_cell_value` 직접 호출이 없어 audit 항목 없음.

#### 질문 / 결정 사항
없음

#### Scope Audit (orchestrator)
pass — 귀속 파일 1건 `tables.py` (frontmatter scope 내). shapes.py 는 audit 결과 무관, 미수정.

#### Verification Re-run (orchestrator)
exit 0 — build_hwpx + showcase 빌드 둘 다 exit 0, syn_div equation=9/text=0 회귀 없음, force_text 동작 OK.

#### Simplify (orchestrator)
SIMPLIFIED=1 — `make_pascal_table` 의 셀 XML 3중 중복 → `_pascal_cell()` 로컬 헬퍼로 추출 (~55줄 제거). VERIFY pass.

#### Review (orchestrator)
VERDICT=pass, ISSUES=0 — 스펙 4-1~4-4 모두 이행, pascal 라우팅 버그 fix 포함, 기존 호출부 하위 호환 유지 (force_text 기본값 False).
