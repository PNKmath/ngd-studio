...
> 2026-05-16 보정: 이 작업은 전체 workflow 대체가 아니라 DeepSeek V4 API 호출 배관 prototype으로 본다. 실제 적용은 [Agent Provider Operating Model](../agent-provider-operating-model/README.md)과 [구현 Roadmap](../agent-provider-operating-model/implementation-roadmap.md)의 typed stage runner 이후에 진행한다.
...
정책은 2026-05-16에 "모든 입력 유형을 외부 API 전송 대상으로 허용"으로 확정되었다. 다만 사용자 선택, stage override, 또는 stage-aware recommendation 없이 자동 전송하지 않는다. 외부 API provider는 파일을 직접 수정하지 않고, 서버가 응답을 검증한 뒤 저장하거나 deterministic runner로 후속 처리를 수행한다.
