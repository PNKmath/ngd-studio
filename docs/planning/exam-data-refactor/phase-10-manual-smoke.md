...
## 체크리스트
- [x] `docs/planning/exam-data-refactor/smoke-runbook.md` 작성 (위 4 시나리오 체크리스트 포함)
...
### 1회차 (2026-05-24 01:35 KST) — completed (runbook 작성)
**상태**: completed
**소요 시간**: 약 3분
**진행 모델**: claude-sonnet-4-6

#### 요약
`smoke-runbook.md` 신규 파일 작성 완료. spec의 설계 섹션에 기술된 4개 시나리오(A, B-α, B-γ, C)를 정리된 체크리스트 형식으로 작성했다. 준비 단계, 시나리오별 확인 항목, 통과 기준, 결과 기록 방법을 포함했다. 실제 PDF smoke 실행은 사용자가 수동으로 수행한다.

#### 변경 파일
- `docs/planning/exam-data-refactor/smoke-runbook.md` (신규, 78줄)

#### 검증 결과
- [x] 파일 존재 및 readable 확인: `ls -lh smoke-runbook.md` → 3.5K, 78줄 — pass
...
pass — docs/planning/exam-data-refactor/smoke-runbook.md 1개 신규, scope 내.

#### Verification Re-run (orchestrator)
skipped — docs-only phase, bash 검증 명령 없음.

#### Simplify (orchestrator)
SIMPLIFIED: 0 — docs 본문 변경 대상 없음.

#### Review (orchestrator)
VERDICT: pass — runbook 4 시나리오(A/B-α/B-γ/C) 체크리스트 포함. NEXT: user_decision — 실제 PDF 통과는 사용자가 수동 실행.
