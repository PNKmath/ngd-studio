...
### 작업 히스토리
- [x] `app/history/page.tsx` 구현
  - [x] 작업 목록 (날짜, 유형 뱃지, 파일명, 상태 도트, 소요시간)
  - [x] 필터: 유형(전체/제작/오검), 상태(전체/완료/실패)
  - [x] 작업 클릭 → 상세 패널 (ID, 입력 파일, followup 내역, 다운로드)

### 작업 큐
- [x] 순차 실행 큐 (`lib/queue.ts` — enqueue, cancelQueued, processNext)
- [x] 큐 상태 표시 (`GET /api/status` — running, queueLength)
- [x] 작업 취소 함수 (cancelQueued)

### 대시보드
- [x] `app/page.tsx` (메인 대시보드)
  - [x] 최근 작업 목록 (최근 5건, 상대시간 표시)
  - [x] 빠른 시작 카드 (제작 / 오검)
  - [x] 시스템 상태 (CLI 연결 + 버전, 대기열 상태)

### 배포
- [x] 팀 서버 배포 스크립트 (`scripts/start.sh` — 의존성 설치 + 빌드 + 실행)
- [x] 환경 변수: HOST, PORT 지원
- [x] 네트워크 접속 URL 자동 표시

### 확인
- [x] `pnpm build` 정상 빌드 (status 라우트 포함)
- [x] 히스토리 필터 + 상세 패널 구현 확인
- [x] 대시보드 시스템 상태 + 최근 작업 구현 확인
