...
- [ ] `POST /api/upload` 라우트
  - [ ] 파일 → `inputs/` 폴더 저장
  - [ ] 파일 메타데이터 반환 (이름, 크기, 경로)

### 파이프라인 뷰어
- [ ] `PipelineView` 컴포넌트
  - [ ] 단계 목록 세로 배치 (5단계 / 오검은 1단계)
  - [ ] 각 단계: 도트(stage color) + 이름 + 상태 + 진행 바
  - [ ] 상태별 스타일: 대기(neutral) → 진행중(info) → 완료(success) → 실패(error)
- [ ] `StageCard` 컴포넌트
  - [ ] 단계 이름, 설명, 진행률(%), 소요시간
  - [ ] 완료 시 결과 요약 텍스트
...
- [ ] `LogStream` 컴포넌트
  - [ ] 다크 배경 터미널 스타일
  - [ ] 자동 스크롤 (하단 고정, 수동 스크롤 시 해제)
  - [ ] 타임스탬프 + 단계 라벨(색상) + 메시지
  - [ ] 접기/펼치기 토글

### 결과 다운로드
- [ ] `GET /api/download/[id]` 라우트
  - [ ] `outputs/` 폴더에서 파일 스트리밍
- [ ] 다운로드 버튼 컴포넌트

### 확인
- [ ] 사이드바 네비게이션 정상 동작
- [ ] 파일 업로드 → inputs/ 저장 확인
- [ ] PipelineView 목 데이터로 렌더링 확인
- [ ] LogStream 목 데이터로 스크롤/색상 확인
...
### 페이지 레이아웃
- [ ] `app/create/page.tsx` 구현
  - [ ] 좌측: 파일 업로드 영역 + 제작 시작 버튼
  - [ ] 우측: 파이프라인 뷰 (5단계)
  - [ ] 하단: 중간 결과 탭 + 로그 스트림

### CLI 연동
- [ ] `POST /api/run` 라우트 구현 (→ `03-api-architecture.md` 참조)
  - [ ] Claude CLI spawn + stream-json 파싱
  - [ ] SSE(Server-Sent Events)로 클라이언트에 스트리밍
  - [ ] 에러 핸들링 (CLI 실패, 타임아웃)
- [ ] 프론트 SSE 연결 + Zustand 스토어 업데이트
- [ ] 프롬프트 조립 로직 (파일 경로 + 작업 지시)

### 스트림 파싱
- [ ] Claude CLI stream-json 출력 → 구조화된 이벤트 변환
  - [ ] 단계 시작/완료 감지
  - [ ] 진행률 추정 로직
  - [ ] 로그 메시지 추출
  - [ ] 에러 메시지 추출
...
### 중간 결과 뷰어
- [ ] 탭 컴포넌트: JSON | 이미지 | 수식
- [ ] JSON 뷰어 (exam_data.json 트리 형태)
- [ ] 이미지 갤러리 (outputs/images/ 의 생성된 그림들)
- [ ] 수식 미리보기 (HWP 수식 스크립트 표시)

### 완료 상태
- [ ] 결과 요약 카드 (문제 수, 그림 수, 소요시간)
- [ ] HWPX 다운로드 버튼
- [ ] 작업 히스토리에 기록

### 확인
- [ ] 실제 PDF + HWPX 업로드 → CLI 실행 → 완료까지 E2E 테스트
- [ ] 파이프라인 단계별 상태 전환 확인
- [ ] 중간 결과 탭 전환 + 내용 표시 확인
- [ ] 에러 발생 시 UI 표시 확인
