...
- [x] coverage-matrix.md의 A11, E1, E2 행에서 본 phase 인용 확인
- [x] `autoValidators.ts` 신규 — 12개 validator 함수 구현 + `runAutoValidators` aggregate
- [x] `mutation.ts` — `ReviewIssueDraft.auto_verified?: boolean` 추가
- [x] `reviewRunner.ts` — autoValidators 결과 통합 + agent에 skipRuleIds 전달
- [x] `__tests__/fixtures/auto-validator-cases/` — 12개 rule × pass/fail = 24 fixture
- [x] `autoValidators.test.ts` — 24 fixture round-trip + 중복 issue 발생 안 함 검증
- [x] `.claude/agents/ngd-exam-reviewer.md` — 자동검증 항목 명시 + skipRuleIds 처리 지침
- [x] **agentic→code 동치성 검증**: 운영 HWPX sample 1건에서
...
- **검증**:
  ...
  - `pnpm vitest run server/review/__tests__/` → 41 tests passed (autoValidators: 33, mutation: 8)
  - `grep -E "skipRuleIds|자동검증 항목" .claude/agents/ngd-exam-reviewer.md` → 4줄 매치
  - `grep -cE "^export function validate[A-Z]" autoValidators.ts` → 12
  - fixture count → 24개 XML
...
#### 요약
autoValidators.ts 신규 작성(12개 결정론적 validator). mutation.ts에 auto_verified 필드 추가. reviewRunner.ts 통합 + skipRuleIds 옵션. ngd-exam-reviewer.md 재작성(자동검증 항목 issue draft 생성 금지). 33개 단위 + 8개 mutation 테스트 + 24개 XML fixture 추가.

#### 검증 결과
- [x] tsc --noEmit → 오류 없음
- [x] server/review/__tests__/ → 41 tests passed
- [x] agent.md skipRuleIds 지침 명시 grep → match
- [x] fixture 24개 확인

#### Scope Audit (orchestrator)
pass — Phase 7 scope edit 모두 정합.

#### Verification Re-run (orchestrator)
exit 0 — 41/41 pass, tsc clean, fixture 24개 확인.

#### Simplify (orchestrator)
SIMPLIFIED: 1 / CHANGES: 2 / VERIFY: pass — autoValidators.ts 미사용 extractScripts 함수 제거.
