...
#### E2E (orchestrator)
partial — build-hwpx-cli pass (시나리오 6단계 모두 PASS: ZIP open 20 members + testzip PASS, section0.xml 유효 XML, BinData 8 entries, hp: namespace 1개, "2024년" 포함). create-v4-full-pipeline 은 사용자 결정으로 skip (dev server + 수동 PDF 업로드 + AI 호출 비용 — intervention 필요). fixture: outputs/_fixtures/year-2024-test/*.hwpx
