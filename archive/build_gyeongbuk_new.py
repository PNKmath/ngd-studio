#!/usr/bin/env python3
"""
NGD HWPX Builder — 경북고 2025 2학년 1학기 기말 수학 II 전범위
...
# Also fix the charPrIDRef for the header template (uses 10, 11, 12 but we need mapping)
# Template uses charPrIDRef="10"(year/semester), "12"(school), "11"(grade/subject and range)
# These map to our charPrIDRef: 3(year), 6(school), 5(grade/range)
# The template already has the right refs baked in from the sample.
...
for prob in problems:
    ...
    # For failed problems (12, 15): treat as choice but use placeholder
    if prob.get("status") == "fail":
        ptype = "choice"
        if not parts:
            # Placeholder text for manual review
            parts = [{"t": f"[문제 {num}] ※ 수동 검수 필요 — solver/verifier 미완성"}]
        if not answer:
            answer = "?"
        if not score:
            score = "4.5"
    ...
    # For choice: endNote + parts content (parts already contain score in some)
    # For essay: endNote + parts content (score is separate [N점] right-aligned)
...
# Build extra image items for HPF
# The template already has image3~image8, but we only have 3 figures -> image3, image4, image5
# We need to customize the HPF to only include the images we actually use
# First build a fresh HPF without the template's extra images

# Base HPF structure
hpf_xml = '''<?xml version="1.0" encoding="UTF-8" standalone="yes" ?><opf:package xmlns:ha="http://www.hancom.co.kr/hwpml/2011/app" xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph" xmlns:hp10="http://www.hancom.co.kr/hwpml/2016/paragraph" xmlns:hs="http://www.hancom.co.kr/hwpml/2011/section" xmlns:hc="http://www.hancom.co.kr/hwpml/2011/core" xmlns:hh="http://www.hancom.co.kr/hwpml/2011/head" xmlns:hhs="http://www.hancom.co.kr/hwpml/2011/history" xmlns:hm="http://www.hancom.co.kr/hwpml/2011/master-page" xmlns:hpf="http://www.hancom.co.kr/schema/2011/hpf" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf/" xmlns:ooxmlchart="http://www.hancom.co.kr/hwpml/2016/ooxmlchart" xmlns:hwpunitchar="http://www.hancom.co.kr/hwpml/2016/HwpUnitChar" xmlns:epub="http://www.idpf.org/2007/ops" xmlns:config="urn:oasis:names:tc:opendocument:xmlns:config:1.0" version="" unique-identifier="" id=""><opf:metadata><opf:title xml:space="preserve">① </opf:title><opf:language>ko</opf:language><opf:meta name="creator" content="text">user</opf:meta><opf:meta name="subject" content="text"/><opf:meta name="description" content="text"/><opf:meta name="lastsaveby" content="text">abund</opf:meta><opf:meta name="CreatedDate" content="text">2024-10-04T11:21:12Z</opf:meta><opf:meta name="ModifiedDate" content="text">''' + MODIFIED_DATE + '''</opf:meta><opf:meta name="date" content="text">2017년 8월 3일 목요일 오후 4:55:39</opf:meta><opf:meta name="keyword" content="text"/></opf:metadata><opf:manifest><opf:item id="header" href="Contents/header.xml" media-type="application/xml"/><opf:item id="image1" href="BinData/image1.bmp" media-type="image/bmp" isEmbeded="1"/><opf:item id="masterpage0" href="Contents/masterpage0.xml" media-type="application/xml"/><opf:item id="image2" href="BinData/image2.bmp" media-type="image/bmp" isEmbeded="1"/>'''
...
hpf_xml += '''<opf:item id="section0" href="Contents/section0.xml" media-type="application/xml"/><opf:item id="settings" href="settings.xml" media-type="application/xml"/></opf:manifest><opf:spine><opf:itemref idref="header" linear="yes"/><opf:itemref idref="section0" linear="yes"/></opf:spine></opf:package>'''
